...
def _main_row(record: ExpenseRecord) -> List[object]:
    return [
        ...
        record.document_date,
        record.document_type,
        record.high_level_category if record.include_in_amount else "",
        ...
        record.origin,
        record.destination,
        record.train_departure_time,
        _float_or_blank(record.refund_fee),
        _float_or_blank(record.change_fee),
        record.description,
        record.original_name,
        record.new_name,
        str(record.source_path),
...
def _review_card(record: ExpenseRecord) -> dict:
    return {
        ...
        "细分类别": record.reimbursement_category if record.include_in_amount else "",
        "报销大类": record.high_level_category if record.include_in_amount else "",
        ...
        "发车时间": record.train_departure_time,
        "退票费": _float_or_blank(record.refund_fee),
        "改签费": _float_or_blank(record.change_fee),
